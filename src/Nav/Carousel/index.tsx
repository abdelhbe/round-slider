...
import navigationStyles from '../../components/common/Navigation/Navigation.module.scss'
import bulletStyles from '../../components/common/Bullet/Bullet.module.scss'
...
class Carousel extends Component<IProps, IState> {
  ...
  calcDicstanceFactor(index:number, offset:number) {
    ...
    const distanceFactor = 1 - Math.abs(thisOffsetFromCenter / (thisOffset + 1));
    ...
    return (
      <React.Fragment>
        ...
        <NavigationButtons>
          ...
          <ul className={navigationStyles.dotNavigator}>
					  {
              this.getPresentableSlides().map(
                (slide: Slide, presentableIndex: number) => (
                  <li
                    ...
                    bulletStyles.bullet,
                    presentableIndex === this.props.goToSlide ? bulletStyles.active : null
                  )}
                  >
